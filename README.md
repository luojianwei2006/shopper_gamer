...
### 通用 WebView / Generic WebView

用 UMG `WebBrowser` 控件（CEF 内核）**应用内**打开任意网页 URL（活动页、公告、支付跳转等），而不是 `Open URL`（那会跳出 App 开系统浏览器，回不来 UI 流）。
...
- **`UShopperWebView`**（`ShopperWebView.h/.cpp`，`UUserWidget` 基类）：C++ 运行时创建 `UWebBrowser`、绑定 `OnUrlChanged` 检测**回调地址特征串**（`ReturnUrlMarker`，可选）。命中广播 `OnWebViewReturn` 并自动 `CloseWebView`。蓝图子类 `WBP_WebView` 只需放一个命名为 **`WebBrowserSlot`** 的 `Overlay` 作容器 + 背景遮罩 + 关闭按钮；不传特征串时纯展示、不自动回调。
- **`UShopperWebSubsystem`**（`ShopperWebSubsystem.h/.cpp`，`UGameInstanceSubsystem`）：`ShowWebView(Url, ReturnUrlMarker?)` 创建并加载网页，`OnWebViewCompleted` 广播最终回调 URL；`WebViewWidgetClass` / `DefaultReturnUrlMarker` 在 CDO 配一次（指到 `WBP_WebView`）。游戏逻辑绑 `OnWebViewCompleted` 做后续处理（如支付回跳后调 `SendGetWallet` 刷新余额）。未实现订单查询——按需自行扩展。
- ⚠️ Mac 编辑器里 CEF 偶尔拉不起 WebView（已知怪癖），**Win / Android / iOS 真机正常**；回调特征串需和对应平台后台配置的 return URL 对齐。
